...
internal static class Bootstrapper
{
    ...
    private static int RunInstaller()
    {
        ...
        try
        {
            ...
            progress = new Form { Text = "SAP MCP Desktop Bridge", Width = 430, Height = 145, StartPosition = FormStartPosition.CenterScreen, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false, ControlBox = false, ShowInTaskbar = true, TopMost = true };
            ...
            using (var process = Process.Start(start))
            {
                ...
                {
                    MessageBox.Show(String.IsNullOrWhiteSpace(error) ? "Installation did not complete." : error, "Installation failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            MessageBox.Show(error.Message, "SAP MCP Desktop Bridge installation failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return 1;
        }
        finally
        {
